...
import { ACCENT, ACCENT2, CARD, ROUTINE_META, ROUTINE_ORDER, SUB, TEXT, pickCelebration } from "@/lib/constants";
...
export function BigMode({
  ...
  const playAgain = () => {
    setSession((s) => s.map((t) => ({ ...t, status: "pending" as const })));
    setIdx(0);
    setFinished(false);
  };
  ...
  if (!available.length)
    return (
      <div style={{ padding: "60px 24px", textAlign: "center" }}>
        ...
        <p style={{ color: SUB, fontSize: 14, lineHeight: 1.6 }}>
          Build a routine in the <strong>Build</strong> tab first.
        </p>
      </div>
    );
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {available.map((rt) => (
            <button
              key={rt}
              onClick={() => start(rt)}
              style={{ padding: "36px 20px", borderRadius: 28, border: "none", background: `linear-gradient(135deg,${ACCENT},${ACCENT2})`, color: "#fff", fontSize: 28, fontWeight: 900, cursor: "pointer", boxShadow: `0 10px 28px ${ACCENT}55` }}
            >
              {ROUTINE_META[rt].emoji}
              <br />
              {ROUTINE_META[rt].label}
            </button>
          ))}
        </div>
      </div>
  ...
  if (finished) {
    ...
    return (
      <div style={{ minHeight: "calc(100vh - 130px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 28, textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 700, color: ACCENT, marginBottom: 6, position: "relative", zIndex: 1 }}>+{doneCount} ⭐ earned</div>
        ...
        <button onClick={backToPicker} style={{ width: "100%", maxWidth: 320, padding: "16px", borderRadius: 24, border: "none", background: "#fff", color: SUB, fontSize: 15, fontWeight: 800, cursor: "pointer", position: "relative", zIndex: 1 }}>
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 130px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", padding: "16px 20px 28px", position: "relative" }}>
      <button onClick={stopEarly} style={{ position: "absolute", top: 8, left: 8, width: 36, height: 36, borderRadius: "50%", border: "none", background: "rgba(0,0,0,0.05)", color: SUB, fontSize: 16, cursor: "pointer", zIndex: 2 }}>
        ‹
      </button>
      ...
      <div style={{ width: "100%", maxWidth: 380 }}>
        <button
          ...
          style={{ width: "100%", padding: "32px", borderRadius: 32, border: "none", background: `linear-gradient(135deg,${ACCENT},${ACCENT2})`, color: "#fff", fontSize: 28, fontWeight: 900, cursor: "pointer", boxShadow: `0 12px 30px ${ACCENT}55`, letterSpacing: 1 }}
        >
          ✅ DONE!
        </button>
