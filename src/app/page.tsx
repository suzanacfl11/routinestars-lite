...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG }}>
      {/* Header — deep navy to plum gradient */}
      <div style={{ background: `linear-gradient(135deg,${ACCENT} 0%,${ACCENT2} 100%)`, padding: "16px 16px 14px" }}>
        <div style={{ marginBottom: 12 }}>
          <ProfileBar profiles={profiles} activeId={activeId}
            onSwitch={setActiveId} onAdd={addProfile}
            onDelete={deleteProfile} onRename={renameProfile}/>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          <StarLogo size={38}/>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 900, fontSize: 18, color: "#fff", letterSpacing: -0.3 }}>RoutineStars Lite</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.8)" }}>Building for {profile.name}</div>
          </div>
